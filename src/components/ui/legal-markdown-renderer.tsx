...
import { Copy, Check, ExternalLink, Scale, FileText, BookOpen } from "lucide-react";
...
export function LegalMarkdownRenderer({
  ...
  const components = useMemo(() => ({
    // Headings with legal document styling
    ...
    // Paragraphs with legal document spacing
    ...
    // Enhanced blockquotes for legal citations and important notes
    ...
    // Lists with legal document styling
    ul: ({ children, ...props }: any) => (
      <ul className="space-y-2 mb-4 ml-6 list-disc marker:text-primary" {...props}>
        {children}
      </ul>
    ),
    
    ol: ({ children, ...props }: any) => (
      <ol className="space-y-2 mb-4 ml-6 list-decimal marker:text-primary marker:font-semibold" {...props}>
        {children}
      </ol>
    ),
    
    li: ({ children, ...props }: any) => (
      <li className="text-sm md:text-base leading-relaxed text-foreground pl-2" {...props}>
        {children}
      </li>
    ),
    ...
    // Tables with legal document styling
    ...
    hr: ({ ...props }: any) => (
      <hr
        className="my-8 border-0 border-t border-border"
        {...props}
      />
    ),
...
// Export a hook for common legal document formatting
export function useLegalMarkdownFormatter() {
  return useCallback((content: string) => {
    // Add common legal document formatting helpers
